...
namespace Sand.Helpers
{
    ...
    public partial class String
    {
        ...
        #region WuBi(获取汉字的五笔简码)
        /// <summary>
        /// 获取汉字的五笔简码，即首字母缩写
        /// </summary>
        /// <param name="chineseText"></param>
        /// <returns></returns>
        public static string WuBi(string chineseText)
        {
            if (string.IsNullOrWhiteSpace(chineseText))
                return string.Empty;
            var result = new StringBuilder();
            foreach (char text in chineseText)
            {
                ...
                if ((text >= 'a' && text <= 'z') || (text >= 'A' && text <= 'Z') || (text >= '0' && text <= '9'))
                    result.Append(char.ToUpper(text));
                result.AppendFormat("{0}", ResolveWuBi(text));
            }
        ...
        /// <param name="text">单个汉字</param>
        /// <returns></returns>
        private static string ResolveWuBi(char text)
        {
            string[] strList = Const.ChineseWuBi.Split(',');
            for (int i = 0; i < strList.Length; i++)
            {
                string strName = strList[i];
                if (strName.IndexOf(text) > 0)
                {
        ...
        public static string FullPinyin(string hzString)
        {
            return NPinyin.Pinyin.GetPinyin(hzString).Replace(" ", "");
        }
    }
}
