...
namespace Sand.Data
{
    ...
    public class SqlQuery : ISqlQuery, IDisposable
    {
        ...
        public void UpdateBatch<T, TKey>(string sql, List<T> updateList) where T : IEntity<TKey>
        {
            ...
            WriteDbConnection.Execute(sql, updateList);
        }
        ...
        protected void CreateDbConnection()
        {
            ...
            {
                var conn = _sqlConfig.ReadSqlConnectionString.IsEmpty() ? _sqlConfig.SqlConnectionString : _sqlConfig.ReadSqlConnectionString;
        ...
        /// 设置数据库连接
        /// </summary>
        protected void CreateWriteDbConnection()
        {
            if (WriteDbConnection == null)
            {
                if (_sqlConfig.DbType == Domain.Uow.DbType.Mysql)
                {
                    WriteDbConnection = new MySqlConnection(_sqlConfig.SqlConnectionString);
                }
                if (_sqlConfig.DbType == Domain.Uow.DbType.Mssql)
                {
                    WriteDbConnection = new SqlConnection(_sqlConfig.SqlConnectionString);
                }
            }
        }
