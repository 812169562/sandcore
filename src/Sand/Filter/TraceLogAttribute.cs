...
namespace Sand.Filter
{
    ...
    public class TraceLogAttribute : ActionFilterAttribute
    {
        ...
        /// 日志名
        /// </summary>
        public const string TraceLogName = "RequestTraceLog";
        ...
        private ILog GetLog()
        {
            try
            {
                return Log.Log.GetLog(TraceLogName);
            }
            catch
            {
                return Log.Log.Null;
        ...
        private async Task AddFormParams(Microsoft.AspNetCore.Http.HttpRequest request, ILog log)
        {
            ...
            var result = await SandFile.ToStringAsync(request.Body, isCloseStream: false);
        ...
        private void AddResult(ActionExecutedContext context, ILog log)
        {
            if (!(context.Result is ApiResult result))
                return;
            log.Content($"响应消息: { result._message}")
                .Content("响应结果:")
                .Content($"{Json.ToJson(result._data)}");
        }
    }
}
