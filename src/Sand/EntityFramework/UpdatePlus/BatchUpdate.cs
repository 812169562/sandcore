...
";
        ...
        /// <param name="defaultFactory">默认添加</param>
        /// <returns>The number of rows affected.</returns>
        public int Execute<T>(IQueryable<T> query, Expression<Func<T, T>> updateFactory, Expression<Func<T, T>> defaultFactory = null) where T : class
        {
            ...
            if (BatchUpdateManager.InMemoryDbContextFactory != null)
            {
	...
	string.Concat("[", store.Schema, "].[", store.Table, "]");
...
#endif
        {
            ...
            if (isSqlServer|| isPostgreSQL || isMySql || isMySqlPomelo || isSQLite)
            {
